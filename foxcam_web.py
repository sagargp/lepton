...
#COLORMAP = [item for sublist in [(k, k, k) for k in range(0, 256)] for item in sublist]

# constants
IMAGE_WIDTH       = 80
IMAGE_HEIGHT      = 60
NUM_FRAME_BYTES   = 10240
...
class FoxCam(object):
  ...
  def open_device(self):
    self.device = usb.core.find(idVendor=0x04d8, idProduct=0x005f)
    ...
    self.output_endpoint = usb.util.find_descriptor(intf, custom_match=lambda e:
        usb.util.endpoint_direction(e.bEndpointAddress) ==
        usb.util.ENDPOINT_OUT)

    self.input_endpoint = usb.util.find_descriptor(intf, custom_match=lambda e:
        usb.util.endpoint_direction(e.bEndpointAddress) ==
        usb.util.ENDPOINT_IN)

    assert None not in (self.input_endpoint, self.output_endpoint)

  def get_frame(self):
    # send frame request
    req = bytearray(64)
    req[0] = 0x81
    self.device.write(self.output_endpoint, req)
    return self.device.read(self.input_endpoint, NUM_FRAME_BYTES)

  def get_renderable_frame(self, frame):
    output_buf = [0] * 4800
    minVal     = 65535.0
    maxVal     = 0.0
    for col in range(0, 80):
      for row in range(0, 30):
        base = col*2*64 + row*2 + 4
        index = col*60 + row
        val = (frame[base] & 0xff) + (frame[base+1] & 0xff) * 255
        output_buf[index] = val

        if val < minVal: minVal = val
        if val > maxVal: maxVal = val

        val = (frame[base + 64] & 0xff) + (frame[base+64+1] & 0xff) * 255
        output_buf[index+30] = val
    ...
    rgba = bytearray(80*60*3)
    for row in range(0, 80):
      for col in range(0, 60):
        val = int((output_buf[col + row*60] - minVal) * scale)

        if val < 0: val = 0
        elif val > 255: val = 255

        offset = col * 80 + row
        rgba[3*offset + 0] = COLORMAP[int(3*val + 0)]
        rgba[3*offset + 1] = COLORMAP[int(3*val + 1)]
        rgba[3*offset + 2] = COLORMAP[int(3*val + 2)]

    return rgba

  def get_jpeg_frame(self):
    data  = self.get_frame()
    frame = self.get_renderable_frame(data)

    # get a frame of IR data
    image = np.fromstring("".join(map(chr, frame)), np.uint8).reshape(60, 80, 3)
    image = cv2.cvtColor(image, cv2.cv.CV_BGR2RGB)
    scaled = cv2.resize(image, (640, 480))

    # get a frame from the RGB webcam
    rval, cam_frame = self.webcam.read()
    scaled_cam_frame = cv2.resize(cam_frame, (640, 480))

    # combine them into a side-by-side view
    vis = np.concatenate((scaled_cam_frame, scaled), axis=1)
